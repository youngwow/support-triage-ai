"""
The single seam between this application and the external LLM provider.

Everything above this module talks to :class:`SupportsStructuredGeneration` and
knows exactly one failure mode, :class:`LLMUnavailableError`. That is what makes
"correct behaviour when the LLM API is down" testable: no test needs a network,
and no caller needs to know which provider is behind the Protocol.
"""
...
class SupportsStructuredGeneration(Protocol):
    """What callers need from an LLM — lets tests script the answers."""
...
class GeminiAgentClient:
    """Structured generation against the Gemini API, retried and time-boxed."""
...
class NullLLMClient:
    ...
    ``genai.Client(api_key="")`` raises at construction, so an unconfigured
    deployment cannot simply hold a real client. Returning this instead means
    the *entire* application still starts and serves traffic with the LLM
    permanently "down" — which is the same code path as a real outage, and is
    how the PoC is meant to be run without credentials.
    """
